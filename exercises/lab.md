...
- **An Ignition API key per gateway you want to scan.** You already have these: `scripts/setup.sh` generated a key per gateway into `.env` (`IGNITION_API_KEY_LOCAL` / `_DEV` / `_PROD`), unique to your clone, and installed the matching token on each gateway. Nothing key-related is in git — a committed key would be a working credential in every fork. (In the gateway UI you'd do the same by hand: *Config → Security → API Keys → New*, scoped to `Project Scan` and `Config Scan`.)
- **GitHub Environments** for the deploy workflows: `lab-gateway-dev` for `deploy.yml`, `lab-gateway-prod` for `release.yml`. Each needs a secret `IGNITION_API_KEY` (the matching `_DEV` / `_PROD` value from your `.env`).
...
2. Under that environment, *Add secret*: `IGNITION_API_KEY` = the `IGNITION_API_KEY_DEV` value from your `.env` (the **dev gateway's** key, generated by `setup.sh`).
3. Repeat for `lab-gateway-prod` with `IGNITION_API_KEY_PROD` from `.env` (the **prod gateway's** key).

> **API keys are per-gateway, not per-URL.** Each gateway only accepts *its own* key; the dev key won't authenticate against prod. The host ports (8089/8090) are just how *you* reach the UIs — the **runner** reaches the same gateways by their compose service names (`http://ignition-dev:8088`, `http://ignition-prod:8088`), which is why `IGNITION_URL`'s default differs from the host port.
