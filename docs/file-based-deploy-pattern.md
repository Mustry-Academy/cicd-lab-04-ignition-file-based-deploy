...
- **Wrong API key for the wrong gateway.** Each lab gateway has its own API key. `IGNITION_API_KEY_LOCAL` won't authenticate against `ignition-test`. The workflow's environment-scoped secrets get this right automatically; manual `scan.sh` calls need the matching environment argument (`scripts/scan.sh test`).
...
Everything *else* identity-related is ordinary deployable config — and in real projects you **do** deploy it. A database or AD user source, a SAML/OIDC identity provider: these hold no password data (the users live in the external system), so they are tracked, deployed, and wiped like any other resource. The same goes for `security-properties/`: it is permission *policy* (APIToken scan grants, designer/config permissions), and this lab commits and deploys it. That's safe because the committed copy points `systemAuthProfile` at `default`, which exists on every gateway — commissioning creates the `default` user source everywhere. Three supporting mechanisms keep it that way. `scripts/setup.sh` stashes the committed `security-properties` aside on any boot where the *local* gateway commissions (otherwise commissioning, finding a security-properties without a matching user source — or with identity files that outlived their data volume — invents a `temp_N` identity and rewrites the file to point at it), lets commissioning create `default`, then restores the file and restarts local. It decides "will it commission?" from the gateway's **data volume**, not from the config tree, because that is what the gateway itself goes by: a volume that disappears without the gitignored identity dirs going with it is exactly how a `temp_N` profile gets created. If one already exists, setup.sh repairs it (restores the committed policy, deletes the temp identity, restarts) — and `validate.sh`/`ci.yml` fail the build if a `temp_N` profile ever reaches a commit, because deploying an auth policy that names a profile only one machine has is a lockout. Finally, `scripts/teardown.sh --volumes` deletes the local gateway's gitignored internal-identity dirs (temp ones included), so a wiped stack recommissions cleanly, like a fresh clone. The design rule: **wipe = repo-owned; preserve = whatever `.deployignore` prunes.**
